...
app.controller("talketrl", ["$scope", "$http", "$mdSidenav", "$mdToast", function($scope, $http, $mdSidenav, $mdToast){
	...
	$scope.signUpUser = function(phone_number)
	{	
		if (phone_number===undefined || phone_number.length != 10) {
			phone_number = phone_number === undefined ? "This" : phone_number;
			$mdToast.show(
				$mdToast.simple()
				.position('end')
				.textContent(phone_number + " is not a valid phone number")
				.hideDelay(3000)
			);
		} else {
			$http({
